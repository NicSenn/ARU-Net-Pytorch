...
class DoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels, filter_size, activation):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size = filter_size, stride = 1, bias=False, padding='same'),
            activation,

            nn.Conv2d(out_channels, out_channels, kernel_size = filter_size, stride = 1, bias=False, padding='same'),
            activation,
        )
    def forward(self, x):
        return self.conv(x)
...
class UNET(nn.Module):
    def __init__(self, in_channels, out_channels, scale_space_num, res_depth, featRoot, filter_size, pool_size, activation):
        ...
        self.bottleneck = DoubleConv(features[-1], features[-1] * 2, filter_size, activation)
        ...
        for feature in reversed(features):
            ...
            self.ups.append(
               ...
               feature * self.pool_size, feature, kernel_size = filter_size, stride=2,
               )
            )

            self.ups.append(DoubleConv(feature* self.pool_size, feature, filter_size, activation))

            

        # last step of the up part (64 -> 1 (in the paper they use 64 -> 2, but we have only 1 out channel))
        self.final_conv = nn.Conv2d(features[0], out_channels, kernel_size=4, stride=1, padding='same') # in the ARU-Net version a kernel size of 4 is used for the final conv of the U-Net
    ...
    def forward(self, x):
        ...
        for down in self.downs:
            ...
            max_pooling = pooling_same_padding(x, kernel_size = self.pool_size, stride = self.pool_size, padding="SAME", poolingType='MAX')
            x = max_pooling(x)
    ...
    def __init__ (self, out_channels, res_depth, filter_size, activation):
        ...
        for aRes in range(0, res_depth):
            if aRes < res_depth-1:  # no activation for last layer
                self.additionalConvs.append(nn.Conv2d(out_channels, out_channels, kernel_size = filter_size, stride = 1, bias=False, padding='same'))
...
class RU_step(nn.Module):
    def __init__(self, in_channels, out_channels, res_depth, filter_size, activation):
        ...
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size = filter_size, stride = 1, bias=False, padding='same')
        self.activation = activation

        self.additionalConvs = RU_additionalConvs(out_channels, res_depth, filter_size, activation)
...
class RU_deconv(nn.Module):
    def __init__(self, in_channels, out_channels, filter_size, activation):
        ...
        self.deconv = nn.ConvTranspose2d(in_channels, out_channels, kernel_size = filter_size, stride = 2)
        self.activation = activation
...
class RU_finalConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.final_conv = nn.Conv2d(in_channels, out_channels, kernel_size=4, stride=1, padding='same')
...
class RUNET(nn.Module):
    def __init__(self, in_channels, out_channels, scale_space_num, res_depth, featRoot, filter_size, pool_size, activation, final_conv_bool):
        ...
        # Down part of RUNET
        for feature in features:
            self.ru_downs.append(RU_step(in_channels, feature, self.res_depth, self.filter_size, self.activation))
            in_channels = feature

        # bottom part of the RU-Net, features[-1] is the last feature
        self.ru_bottleneck = RU_step(features[-1], features[-1] * self.pool_size, self.res_depth, self.filter_size, self.activation)
        ...
        for feature in reversed(features):
            ...
            self.ru_ups.append(
                # in_channels = feature*2 because we add the skip connection, i.e. 512 -> 1024
                # out_channels = feature
                RU_deconv(feature * self.pool_size, feature, self.filter_size, self.activation)
            )

            self.ru_ups.append(RU_step(feature * self.pool_size, feature, self.res_depth, self.filter_size, self.activation))
...
class ANET(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in self.a_layers:
            ...
            if(counter <= 3):
                maxpooling = pooling_same_padding(x, kernel_size = 2, stride = 2, padding='SAME', poolingType='MAX')
                x = maxpooling(x)
            counter += 1
...
class ARUNET(nn.Module):
    def __init__(self, in_channels=3, out_channels=1, scale_space_num = 6, res_depth = 3, featRoot = 8, filter_size = 3, pool_size = 2, activation = nn.ReLU(), num_scales = 5):
        ...
        self.ru_net = RUNET(in_channels=in_channels, out_channels = out_channels, scale_space_num = self.scale_space_num, res_depth = self.res_depth, 
        featRoot = self.featRoot, filter_size=self.filter_size, pool_size=self.pool_size, activation=self.activation, final_conv_bool = False)
        ...
        for sc in range(0, self.num_scales - 1):
            upSc = upSc * 2
            self.ru_net_upsample.append(nn.ConvTranspose2d(in_channels = self.featRoot, out_channels = self.featRoot, kernel_size = upSc, stride = upSc))

        self.a_net_upsample = nn.ModuleList()
        upSc = 8
        for sc in range(0, self.num_scales):
            self.a_net_upsample.append(nn.ConvTranspose2d(in_channels = 1, out_channels = 1, kernel_size = upSc, stride = upSc))
            upSc = upSc * 2

        self.a_net_softmax = nn.Softmax(dim = 1)

        self.final_conv = nn.Conv2d(self.featRoot, out_channels, kernel_size = 4, stride = 1, padding = 'same')
    ...
    def forward(self, x):
        ...
        for sc in range(1, self.num_scales):
            avgpool = pooling_same_padding(scaled_inputs[sc-1], kernel_size = 2, stride = 2, padding='SAME', poolingType='AVG')
...
def pooling_same_padding(input, kernel_size, stride, padding='SAME', dilation=1, poolingType='AVG'):
    """
    ...
    if(poolingType == "AVG"):
        return nn.AvgPool2d(kernel_size = kernel_size, stride = stride, padding=padding).cuda()
    elif(poolingType == "MAX"):
        return nn.MaxPool2d(kernel_size = kernel_size, stride = stride, padding=padding, dilation=dilation).cuda()
    else:
...
def create_aru_net(in_channels = 3, out_channels = 1, model_kwargs = {}):
    ...
    if 'aru' in model:
        print("Using ARU-Net")
        return ARUNET(in_channels=in_channels, out_channels=out_channels, scale_space_num = scale_space_num, res_depth = res_depth, 
        featRoot=featRoot, filter_size=filter_size, pool_size=pool_size, activation=activation, num_scales=num_scales)
    elif 'ru' in model:
        print("Using RU-Net")
        return RUNET(in_channels=in_channels, out_channels = out_channels, scale_space_num = scale_space_num, res_depth = res_depth, 
        featRoot = featRoot, filter_size=filter_size, pool_size=pool_size, activation=activation, final_conv_bool=True)
    else:
        print("Using U-Net")
        return UNET(in_channels=in_channels, out_channels = out_channels, scale_space_num = scale_space_num, res_depth = res_depth, 
        featRoot = featRoot, filter_size=filter_size, pool_size=pool_size, activation=activation)
